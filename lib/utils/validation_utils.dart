...
String? emptyNotAllow({dynamic value,String errorMessage = "Empty not allowed", bool isRequired = true}) {
  if (isRequired && value != null && value.toString().trim().isNotEmpty) {
    return null;
  }
  return errorMessage;
}
...
String? validatePassword(value) {
  if (value != null && value.trim().length > 5) {
    ...
    return "Enter password at least six character";
  }
}
...
String? matchPassword(value, String oldPassword) {
  if (value != null && value.trim().length > 5 && oldPassword.trim() == value.trim()) {
    return null;
  } else {
...
String? validateMobileNumber(value, {bool isNotRequired = true}) {
  if (isNotRequired && (value == null || value.trim().length != 10)) {
