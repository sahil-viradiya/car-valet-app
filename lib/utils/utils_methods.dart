...
import 'package:wallet_parking/utils/app_snackbar.dart';
import 'package:wallet_parking/utils/static_data.dart';
import 'package:wallet_parking/utils/validation_utils.dart';
  ...
  Get.defaultDialog(
    ...
    confirm: GestureDetector(
      onTap: () async {
        ...
        StaticData.selectedPreIndex.value = 1;
...
int convertDateOnlyFromMilli(int milliSeconds) {
  var currentDate = DateFormat('yyyy-MM-dd').format(DateTime.fromMillisecondsSinceEpoch(milliSeconds));
  return DateTime.parse(currentDate).millisecondsSinceEpoch;
}
...
Future<bool> showExitPopup(BuildContext context, GlobalKey<ScaffoldState> scaffoldKey) async {
          ...
          children: [
            ...
            Container(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  "yes".tr,
                  style: const TextStyle(fontFamily: 'Poppins', fontSize: 17, color: Colors.white),
                ),
              ),
            ),
...
dynamic getDocumentValue({required DocumentSnapshot? documentSnapshot, required String key, dynamic defaultValue = ""}) {
  if (documentSnapshot != null && documentSnapshot.exists && (documentSnapshot.data() as Map<String, dynamic>).containsKey(key)) {
...
String getDatTimeString(int millisecond){
  ...
  var hoursMin = DateFormat('hh:mm a').format(DateTime.fromMillisecondsSinceEpoch(millisecond));
  if(today.isSameDate(dateOnly)){
    ...
    timeString = DateFormat('dd-MM-yyyy hh:mm a').format(DateTime.fromMillisecondsSinceEpoch(millisecond));
  }
  return timeString;
}
